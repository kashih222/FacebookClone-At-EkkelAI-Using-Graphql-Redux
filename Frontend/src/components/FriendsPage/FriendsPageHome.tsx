...
const FriendsPageHome = () => {
  ...
  const loadMixedData = async () => {
    try {
      ...
      const friends: FriendData[] =
        friendsJson.data?.myFriends?.map((friend: FriendData) => ({
          id: friend.id,
      ...
      const friendRequests: FriendRequestData[] =
        requestsJson.data?.friendRequests?.map(
  ...
  const handleAcceptRequest = async (requestId: string) => {
    try {
      ...
      if (json.errors && json.errors.length) {
        toast.error(
          json.errors[0].message || "Failed to accept friend request"
        );
        return;
      }
      // Refresh the data
  ...
  const handleDeleteRequest = async (requestId: string) => {
    try {
      ...
      if (json.errors && json.errors.length) {
        toast.error(
          json.errors[0].message || "Failed to reject friend request"
        );
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-100  ">
      ...
      <div className="flex">
        ...
        {/* Main Content */}
        <div className="flex-1 p-3 sm:p-4 md:p-6 lg:p-8 w-full mt-24 lg:mt-0">
          {/* Header */}
          <div className="flex items-center justify-between mb-4 sm:mb-6 mt-16 ">
          ...
          {loading ? (
            ...
            <div className="text-center py-8 sm:py-12">
              ...
              <p className="text-gray-500 text-sm sm:text-base px-4">
                {activeTab === "all"
                  ? "When you have friends or requests, they'll appear here."
                  : activeTab === "friends"
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
                {filteredData.map((item) => {
                  const isRequest =
                    "isFriendRequest" in item && item.isFriendRequest;
                  const firstName = isRequest
                    ? item.from.firstName
                    : item.firstName;
